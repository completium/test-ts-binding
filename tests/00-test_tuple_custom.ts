import * as ex from "@completium/experiment-ts";
import { Nat, Bytes } from "@completium/archetype-ts-types";

import { tuple_custom } from './binding/tuple_custom'

const assert = require('assert')

/* Accounts ---------------------------------------------------------------- */

const alice = ex.get_account('alice');

/* Endpoint ---------------------------------------------------------------- */

ex.set_mockup()

/* Verbose mode ------------------------------------------------------------ */

ex.set_quiet(true);

/* Now --------------------------------------------------------------------- */

ex.set_mockup_now(new Date(Date.now()))

/* Scenario ---------------------------------------------------------------- */

describe('[TUPLE_CUSTOM] Contract deployment', async () => {
  it('Deploy test_binding', async () => {
    await tuple_custom.deploy({ as: alice })
  });
})

describe('[TUPLE_CUSTOM] Call entry', async () => {
  it("Call 'set_value'", async () => {
    // const res_before = await tuple_custom.get_res()
    // assert(res_before[0].equals(new Nat(0)) && res_before[1] == "" && res_before[2].equals(new Bytes("")), "Invalid Value")
    const v: [Nat, [string, Bytes], boolean] = [new Nat(2), ["toto", new Bytes("ff")], true];
    await tuple_custom.set_value(v, { as: alice })
    const res = await tuple_custom.get_res()
    assert(res[0].equals(v[0]) && res[1][0] == v[1][0] && res[1][1].equals(v[1][1]) && res[2] == v[2], "Invalid Value")

  })
})
